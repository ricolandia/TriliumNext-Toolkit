console.log("🚀 Canvas Linker widget carregado (v7.0)");
/**
 * ╔══════════════════════════════════════════════════════╗
 * ║        Canvas Note Linker — TriliumNext  v7         ║
 * ║        + Gerador de Fluxos (DSL nós + setas)        ║
...
// Funções puras (sem acesso a api/DOM) para permitir teste isolado.
...
const FLOW_CONFIG = {
    ...
    strokeWidth: 2, arrowStrokeWidth: 2,
    arrowLabelFontSize: 12,
...
function flowNodeSize(node) {
    ...
    const base = node.type === 'decisao' ? [cfg.decisionW, cfg.decisionH]
               ...
               : [cfg.nodeW, cfg.nodeH];
    const charsPerLine = Math.max(6, Math.floor((base[0] - cfg.padX * 2) / (cfg.fontSize * 0.62)));
    const wrapped = flowWrapText(node.label, charsPerLine);
    const lines = wrapped.split('\n').length;
    const textH = Math.ceil(lines * cfg.fontSize * cfg.lineHeight);
    return { w: base[0], h: Math.max(base[1], textH + cfg.padY * 2 + 6), wrapped, textH };
}

/**
 * Layout em camadas (longest-path). Suporta ciclos (arestas de retorno não
 * propagam rank além de algumas passadas). Retorna nós posicionados + arestas.
 */
function layoutFlow(nodes, edges, direction = 'TB') {
    ...
    for (const e of edges) { out.get(e.from).push(e.to); indeg.set(e.to, indeg.get(e.to) + 1); }
    ...
    const inCycle = new Set(nodes.map(n => n.id).filter(id => !ordered.includes(id)));
    for (let pass = 0; pass < 4; pass++) {
        for (const id of inCycle) {
            let r = rank.get(id);
            for (const e of edges) {
                if (e.to === id && !inCycle.has(e.from)) r = Math.max(r, rank.get(e.from) + 1);
            }
            rank.set(id, r);
        }
    }
    ...
    // agrupa por rank e ordena por posição média dos pais (menos cruzamentos)
    const ranks = new Map();
    ...
    for (const rk of rankKeys) {
        ...
        if (rk !== rankKeys[0]) {
            const avg = new Map();
            for (const id of ids) {
                const parents = edges.filter(e => e.to === id).map(e => posIndex.get(e.from)).filter(v => v !== undefined);
                avg.set(id, parents.length ? parents.reduce((a, b) => a + b, 0) / parents.length : posIndex.size);
            }
            ids.sort((a, b) => (avg.get(a) - avg.get(b)) || a.localeCompare(b));
        }
        ids.forEach((id, i) => posIndex.set(id, i));
    }

    // posições
    const placed = new Map();
    let cursor = 0;       // eixo principal (rank)
    let laneMax = 0;      // extensão no eixo secundário da faixa atual
    const gapMain = direction === 'TB' ? cfg.gapY : cfg.gapX;
    const gapCross = direction === 'TB' ? cfg.gapX : cfg.gapY;

    for (const rk of rankKeys) {
        const ids = ranks.get(rk);
        const crossSizes = ids.map(id => {
            const s = sizes.get(id);
            return direction === 'TB' ? s.w : s.h;
        });
        const totalCross = crossSizes.reduce((a, b) => a + b, 0) + gapCross * Math.max(0, ids.length - 1);
        const mainSize = Math.max(...ids.map(id => {
            const s = sizes.get(id);
            return direction === 'TB' ? s.h : s.w;
        }));

        let cross = -totalCross / 2;
        for (const id of ids) {
            const s = sizes.get(id);
            const cw = direction === 'TB' ? s.w : s.h;
            const ch = direction === 'TB' ? s.h : s.w;
            const x = direction === 'TB' ? cross : cursor;
            const y = direction === 'TB' ? cursor : cross;
            placed.set(id, {
                ...byId.get(id), x, y, w: cw, h: ch,
                textW: s.wrapped, textH: s.textH, rank: rank.get(id),
            });
            cross += cw + gapCross;
        }

        cursor += mainSize + gapMain;
    }

    // normaliza para origem 0,0
    const xs = [...placed.values()].map(n => n.x);
    const ys = [...placed.values()].map(n => n.y);
    const dx = xs.length ? Math.min(...xs) : 0;
    const dy = ys.length ? Math.min(...ys) : 0;
    for (const n of placed.values()) { n.x -= dx; n.y -= dy; }
...
function flowArrowEl(o) {
    const cfg = FLOW_CONFIG;
    const minX = Math.min(o.sx, o.ex);
    const minY = Math.min(o.sy, o.ey);
    return {
        id: o.id, type: 'arrow',
        x: minX, y: minY,
        width: Math.abs(o.ex - o.sx), height: Math.abs(o.ey - o.sy), angle: 0,
        strokeColor: '#1e1e2e', backgroundColor: 'transparent',
        fillStyle: 'solid', strokeWidth: cfg.arrowStrokeWidth, strokeStyle: 'solid',
        roughness: cfg.roughness, opacity: 100,
        ...
        boundElements: [],
        updated: Date.now(), link: null, locked: false,
        points: [[o.sx - minX, o.sy - minY], [o.ex - minX, o.ey - minY]],
        lastCommittedPoint: null,
        startBinding: { elementId: o.fromElId, focus: 0, gap: 4 },
        endBinding:   { elementId: o.toElId,   focus: 0, gap: 4 },
...
/**
 * Gera os elementos Excalidraw do fluxo. `origin` desloca o conjunto todo.
 * Retorna { elements, width, height } — width/height = bbox local.
 */
function buildFlowElements(nodes, edges, direction = 'TB', origin = { x: 0, y: 0 }) {
    const cfg = FLOW_CONFIG;
    const { nodes: placed, edges: validEdges } = layoutFlow(nodes, edges, direction);

    const els = [];
    const shapeIdOf = new Map();
    const byId = new Map(placed.map(n => [n.id, n]));
    let seq = 0;
    const nextIndex = () => 'a' + (++seq);
    ...
    for (const n of placed) {
        ...
        shapeIdOf.set(n.id, shapeId);

        const shapeType = n.type === 'decisao' ? 'diamond' : 'rectangle';
        const shape = flowBaseEl(shapeType, {
            id: shapeId,
        ...
        const text = flowTextEl({
            id: 'flw_t_' + Math.random().toString(36).slice(2, 14),
            x: origin.x + n.x + cfg.padX,
            y: origin.y + n.y + Math.max(2, (n.h - n.textH) / 2),
            width: n.w - cfg.padX * 2,
            height: n.textH,
            text: n.textW, fontSize: cfg.fontSize,
    ...
    for (const e of validEdges) {
        ...
        if (direction === 'TB') {
            if (to.rank > from.rank) {
                ...
                sx = from.x + from.w;     sy = from.y + from.h / 2;
                ex = to.x + to.w / 2;     ey = to.y + to.h;
            }
        } else {
            if (to.rank > from.rank) {
                sx = from.x + from.w;     sy = from.y + from.h / 2;
                ex = to.x;                ey = to.y + to.h / 2;
            } else {
                sx = from.x + from.w / 2; sy = from.y + from.h;
                ex = to.x + to.w / 2;     ey = to.y;
            }
        }
        sx += origin.x; sy += origin.y; ex += origin.x; ey += origin.y;

        const arrow = flowArrowEl({
            id: 'flw_a_' + Math.random().toString(36).slice(2, 14),
            sx, sy, ex, ey,
            fromElId: shapeIdOf.get(e.from), toElId: shapeIdOf.get(e.to),
            index: nextIndex(),
        });
        els.push(arrow);
        grow(Math.min(sx, ex), Math.min(sy, ey), Math.abs(ex - sx), Math.abs(ey - sy));

        const fromShape = els.find(x => x.id === arrow.startBinding.elementId);
        const toShape   = els.find(x => x.id === arrow.endBinding.elementId);
        if (fromShape) fromShape.boundElements.push({ type: 'arrow', id: arrow.id });
        if (toShape)   toShape.boundElements.push({ type: 'arrow', id: arrow.id });
        ...
        if (e.label) {
            ...
            els.push(flowTextEl({
                id: 'flw_l_' + Math.random().toString(36).slice(2, 14),
                x: midX + (isHorizontal ? 0 : 8),
                y: midY + (isHorizontal ? -22 : 0),
                width: 120, height: 18,
                text: e.label,
    ...
    return { elements: els, width, height };
}
// ── FLOW ENGINE (fim) ───────────────────────────────────
...
class CanvasLinkerWidget extends api.NoteContextAwareWidget {
    ...
    _injectFloat() {
        ...
        <div id="clw-root">
            ...
            <div id="clw-flow-panel" class="clw-panel clw-panel--flow" style="width:400px">
                <div class="clw-panel-header">
                    <span class="clw-panel-icon">🪄</span>
                    <span class="clw-panel-title">Gerar fluxo</span>
                    <button id="clw-flow-close" class="clw-panel-close">✕</button>
                </div>
                <textarea id="clw-flow-spec" class="clw-textarea" rows="9" spellcheck="false"
                    placeholder="Início: Recebe pedido [inicio]&#10;Brief: Tem briefing? [decisao]&#10;&#10;Início -> Brief&#10;Brief -> Orçamento : sim"></textarea>
...
/* ── Gerador de fluxos ── */
.clw-textarea {
    width:100%; min-height:170px; resize:vertical; padding:9px 12px;
...
#clw-flow-panel .clw-status   { display:none; text-align:left; padding:8px 0 0; }
    ...
    _flowScene(parsed) {
        const { elements } = buildFlowElements(parsed.nodes, parsed.edges, parsed.direction, { x: 0, y: 0 });
        return {
            elements,
    ...
    async _generateFlowToCanvas() {
        ...
        try {
            const { elements } = this._flowScene(parsed);
            ...
            this._flowStatus(`✅ ${count} elementos gerados — fluxo ao lado do conteúdo existente.`);

...
#wc-daily-bar { height: 5px; border-radius: 3px; background: var(--main-border-color); margin: 4px 0 8px; }
...
class WordCountWidget extends api.RightPanelWidget {
    ...
    _readGoal() {
        if (this.note) {
            const v = this.note.getLabelValue('dailyGoal');
            if (v) return parseInt(v, 10) || GOAL_DEFAULT;
        }
        return GOAL_DEFAULT;
    }
